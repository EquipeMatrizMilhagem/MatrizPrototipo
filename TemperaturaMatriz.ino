...
#define dhtpin 8
#define ledpin 2
#define temp_minima -5
#define temp_maxima 35
#define intervaloleitura 2000
DHT dht(dhtpin,DHT22);


float temperaturatotal = 0.0;
unsigned long controleleitura = 0.0;
...
#define minvelocidade 50 //Definição da velocidade mínima
#define fadeamount 5 
...
void loop() {
  ...
  while (velocidade < maxvelocidade){
  ...
  for (velocidade = minvelocidade; velocidade <= maxvelocidade;
    velocidade += fadeamount) {
    analogWrite(pinmotor, velocidade);
    senddata(Tempo);
    Tempo = Tempo + 0.25;
  ...
  if (velocidade == 255){
    while(velocidade <=255 && velocidade >=50){
      velocidade-= fadeamount;
      analogWrite(pinmotor, velocidade);
      senddata(Tempo);
      Tempo = Tempo + 0.25;
...
void temperatura(){
  if (millis()- controleleitura > intervaloleitura) {
    float leitura = dht.readTemperature(); // faz a leitura da temperatura
    if (!isnan(leitura)) { // verifica se recebeu algum valor do sensor
      temperaturatotal = leitura;
      Serial.print(F("Temperatura: "));
      Serial.print(temperaturatotal);
      Serial.println(F(" C°"));
      controleleitura = millis();
      if (temperaturatotal > temp_maxima || temperatura < temp_minima) {
